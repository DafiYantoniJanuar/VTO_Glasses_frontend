...
import FaceShapePanel from '../../components/faceShape/FaceShapePanel'
              ...
              <div className="pdp-camera-actions">
                ...
                {cameraActive && (
                  <>
                    ...
                    <button
                      className={`pdp-scan-btn ${isScanning ? 'scanning' : ''} ${detectedFaceShape && showFaceScan ? 'done' : ''}`}
                      onClick={() => {
                        if (!faceDetected) {
                          setToastMessage('Arahkan wajah ke kamera terlebih dahulu')
                          setTimeout(() => setToastMessage(null), 3000)
                          return
                        }
                        setIsScanning(true)
                    ...
                    >
                      ...
                      {isScanning ? 'Scanning...' : 'Scan Wajah'}
                    </button>
                  </>
                )}
              </div>

              {/* Face Scan Result — below camera actions */}
              {showFaceScan && (
                <FaceShapePanel
                  ...
                  onShapeDetected={(shape) => {
                    ...
                    try { localStorage.setItem('vto_detected_face_shape', shape) } catch {}
                  }}
